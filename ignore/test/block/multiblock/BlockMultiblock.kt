...
abstract class BlockMultiblock(material: Material, name: String) : BlockMultiState(material, name) {
    ...
    fun activateMultiblock(context: MultiblockContext) {
        ...
        if (errors.isNotEmpty()) {
            playerIn.sendMessage("text.immersivemagneticraft.multiblock.error_count", errors.size, color = TextFormatting.AQUA)
            if (errors.size > 2) {
                playerIn.sendMessage("text.immersivemagneticraft.multiblock.first_errors", 2, color = TextFormatting.AQUA)
                var count = 0
                errors.forEach {
                ...
                playerIn.sendMessage("text.immersivemagneticraft.multiblock.all_errors", color = TextFormatting.DARK_AQUA)
                errors.forEach {
                    playerIn.addChatComponentMessage(it.apply { style.color = TextFormatting.DARK_RED })
                }
            }
            ...
            playerIn.sendMessage("text.immersivemagneticraft.multiblock.activate", color = TextFormatting.GREEN)
        }
    }
