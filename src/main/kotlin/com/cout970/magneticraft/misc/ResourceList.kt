...
object ResourceList {
    ...
    fun getModAssets(): List<String> {
        ...
        val pattern = """.*assets[/\\]immersivemagneticraft[/\\].*""".toRegex()
    ...
    fun getGuideBookLanguages(): List<String> {
        val file = Magneticraft.sourceFile
        val pattern = """.*assets[/\\]immersivemagneticraft[/\\]guide[/\\].*""".toRegex()
        ...
        return files
            .map {
                it.replace(""".*assets[/\\]immersivemagneticraft[/\\]guide[/\\](.*)[/\\].*""".toRegex(), "$1")
            }
            .distinct()
    }
    ...
    fun getGuideBookPages(lang: String): List<ResourceLocation> {
        val file = Magneticraft.sourceFile
        val pattern = """.*assets[/\\]immersivemagneticraft[/\\]guide[/\\]$lang[/\\].*""".toRegex()
        val files = if (file.isDirectory) {
            getResourcesFromDirectory(file, pattern)
        } else {
            getResourcesFromJarFile(file, pattern)
        ...
        return files.map {
            it.replace(""".*assets[/\\]immersivemagneticraft[/\\]guide[/\\]$lang[/\\](.*)""".toRegex(), "$1")
        }.map { name ->
